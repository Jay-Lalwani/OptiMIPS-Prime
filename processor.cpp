...
void Processor::single_cycle_processor_advance()
{
    ...
    imm = control.zero_extend ? imm : (imm >> 15) ? 0xffff0000 | imm
                                                  : imm;
    ...
    write_data_mem = control.halfword ? (read_data_mem & 0xffff0000) | (read_data_2 & 0xffff) : control.byte ? (read_data_mem & 0xffffff00) | (read_data_2 & 0xff)
                                                                                                             : read_data_2;
    ...
    read_data_mem &= control.halfword ? 0xffff : control.byte ? 0xff
                                                              : 0xffffffff;

    int write_reg = control.link ? 31 : control.reg_dest ? rd
                                                         : rt;

    uint32_t write_data = control.link ? regfile.pc + 8 : control.mem_to_reg ? read_data_mem
                                                                             : alu_result;
    ...
    regfile.pc = control.jump_reg ? read_data_1 : control.jump ? (regfile.pc & 0xf0000000) & (addr << 2)
                                                               : regfile.pc;
}
...
void Processor::pipelined_processor_advance()
{
    ...
    // write back stage
    if (mem_wb_reg.control.reg_write)
    {
        uint32_t write_data = mem_wb_reg.control.mem_to_reg ? mem_wb_reg.mem_read_data : mem_wb_reg.alu_result;
        regfile.access(0, 0, write_data, write_data,
                       mem_wb_reg.write_reg, true, write_data);
    }

    // commit pipeline registers
    // mem_wb_reg = new_mem_wb;
    // ex_mem_reg = new_ex_mem;
    // id_ex_reg = new_id_ex;
    // if_id_reg = new_if_id;
}
